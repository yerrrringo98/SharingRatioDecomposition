...
class Bottleneck(nn.Module):
    ...
    def simprop2(self,o,inference_pkg):
        ...
        o1,o2 = self.add.simprop2(o,inference_pkg)
        o1_part = compute_sharing_ratio(inference_pkg.output_dict[self.bn3.name],inference_pkg.output_dict[self.relu3.name])
        if self.downsample is not None:
            o2_part = compute_sharing_ratio(inference_pkg.output_dict[self.downsample[1].name],inference_pkg.output_dict[self.relu3.name])
        else:
            o2_part = compute_sharing_ratio(block_input.to('cpu'),inference_pkg.output_dict[self.relu3.name])
        o1.simmap = o1_part.to(inference_pkg.device) * o1.simmap
        o2.simmap = o2_part.to(inference_pkg.device) * o2.simmap
        tot_bias = inference_pkg.bn_true_bias_dict[self.bn3.name]
        if self.downsample is not None:
            downsample_bias = inference_pkg.bn_true_bias_dict[self.downsample[1].name]
            tot_bias += downsample_bias

            o2 = self.downsample.simprop2(o2,inference_pkg)
            
            #tot_simmap = bed_of_nail_upsample2d(tot_simmap,(2,2),(0,0))
            _,c,_,_ = inference_pkg.output_dict[self.bn3.name].shape
            residual_input = inference_pkg.output_dict[self.bn3.name].to(inference_pkg.device) - inference_pkg.bn_true_bias_dict[self.bn3.name].reshape(1,c,1,1)
            o2 = o2.simprop2_input(block_input,o2,inference_pkg,residual_input=residual_input,total_bias=tot_bias) # 
            downsample_residual = inference_pkg.output_dict[self.downsample[1].name].to(inference_pkg.device) - inference_pkg.bn_true_bias_dict[self.downsample[1].name].reshape(1,c,1,1)
            downsample_residaul = downsample_residual.to(inference_pkg.device)
        ...
        o1 = self.relu2.simprop2(o1,inference_pkg,residual_input = downsample_residual if self.downsample is not None else block_input, total_bias = tot_bias)#. 이걸로는 bias 분배만 계산할때 사용.. 차라리 bias 영향을 따로 simmap으로 빼는게 나을지도..? residual input은 그냥 자기 자신 보정해주면됨.
    ...
    def simprop2_layer1(self,o,inference_pkg,maxpool,relu):
        ...
        if self.downsample is not None:
            ...
            _k = maxpool.kernel_size
            _s = maxpool.stride
            _p = maxpool.padding
            m = nn.Conv2d(1,1,_k,_s,padding=_p,bias=False).to(inference_pkg.device)
            ...
            #o2 = o2.simprop2_input(block_input,o2,inference_pkg)
            o2 = relu.simprop2(o2,inference_pkg)
            res_simmap = o2.simmap.clone()
        block_input = inference_pkg.input_dict[self.name].to(inference_pkg.device)

        o1.simmap = o1.simmap - res_simmap
        o1 = self.bn3.simprop2(o1,inference_pkg)
        o1 = self.conv3.simprop2(o1,inference_pkg)
