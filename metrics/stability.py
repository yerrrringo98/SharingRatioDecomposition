...
class AverageStability(ExplainerMetric):
    ...
    def __init__(self,
        ...
        if distance == 'l1':
            self.distance = lambda x, y: torch.sum(torch.abs(x - y))
        elif distance == 'l2':
            self.distance = lambda x, y: torch.sum((x-y)**2.0)
        ...
        self.noisy_masks = self.radius * torch.rand((nb_samples, *inputs.shape[1:]))

    def evaluate(self,
                 explainer: Callable,
                 base_explanations: Optional[Union[torch.Tensor, np.ndarray]] = None,
        ) -> float:
        ...
        for inp, label, phi in zip(self.inputs, self.targets, base_explanations):
            ...
            label = torch.repeat_interleave(label[None,:],self.nb_samples,0)
