...
def sensitivity_max(
    ...
    **kwargs: Any,
    ...
    def _next_sensitivity_max(current_n_perturb_samples: int) -> Tensor:
        ...
        #expl_inputs = _format_tensor_into_tuples(expl_inputs)
        ...
        sensitivities = torch.cat(
            [
                ...
                for expl_perturbed, expl_input in zip(
                    expl_perturbed_inputs, expl_inputs_expanded # 둘이 batchsize 같아야함..
                )
            ],
            dim=1,
        )
        ...
        expl_inputs_norm_expanded = torch.norm(
            ...
            dim=1,
            keepdim=True,
        ).repeat_interleave(current_n_perturb_samples, dim=0) # (170,1)
