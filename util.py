...
# def initialize_infer_pkg(net,infer_pkg,data_loader,device = "cpu"):
    ...
    for _c in range(c_i):
        ...
        for _w in range(w):
            for _h in range(h):
                ...
                #true_last_out = last_out - inference_pkg.bn_true_bias_dict[bn.name].reshape((1,out.shape[1],1,1))
                
                
                
                #out = relu.decomp_forward(out,inference_pkg)
...
def cal_simmap_cr(input,o,inference_pkg,cur_layer_name=None):
    ...
    for _w in range(w):
        for _h in range(h):
            ...
            out = F.conv2d(input,weight,torch.zeros_like(bias),stride,padding)
            #out = F.conv2d(input,weight,bias / (w*h),stride,padding)
            true_last_out = last_out - bias.reshape((1,out.shape[1],1,1))
            #out = relu.decomp_forward(out,inference_pkg)
            out_norm = torch.norm(out,dim=1,keepdim=True).detach()
            bias_bunbae = out_norm / (total_norm + 1e-14)
            ...
            simmap *= o.simmap
            simmap = bed_of_nail_upsample2d(simmap,scale_factor=stride,offset = (_h,_w), padding= conv_padding)
            #simmap = bed_of_nail_upsample2d(simmap,scale_factor = stride,offset=(_h,_w), target_shape=simmap_total.shape)
            simmap = simmap[:,padding[0]:H+padding[0],padding[1]:H + padding[1]]
        ...
        for _h in range(h):
            for _w_p in range(w_p):
                for _h_p in range(h_p):
                    ...
                    simmap *= o.simmap
                    #_h,_w,stride,padding에 따라 원래 map으로 돌려놔야함
                    
                    # pad_simmap = torch.zeros_like(simmap_total,requires_grad=True)
                    # pad_simmap = F.pad(pad_simmap,(padding[0],padding[0],padding[1],padding[1]),'constant',0)
                    # 그냥 인덱스 슬라이스 쓰자.
                    #conv upsample
                    simmap = bed_of_nail_upsample2d(simmap,scale_factor=stride,offset = (_h,_w), padding= conv_padding)
...
def cal_simmap_mcbr(input,o,inference_pkg,cur_layer_name=None):
        ...
        for _h in range(h):
                for _w_p in range(w_p):
                    for _h_p in range(h_p):
                        ...
                        # _input[:,:,_w_p::w_p_s,_h_p::h_p_s] = F.pad(input,maxpool_padding,'constant',0)[:,:,_w_p::w_p_s,_h_p::h_p_s]
                        # _input = _input[:,:,maxpool.padding:H+maxpool.padding,maxpool.padding:W+maxpool.padding]
                        #debug += _input
                        # _input = maxpool.decomp_forward(_input,inference_pkg)
    ...
    for _w in range(w):
        for _h in range(h):
            debug = torch.zeros((1,64,56,56),device=inference_pkg.device)
            for _w_p in range(w_p):
                for _h_p in range(h_p):
